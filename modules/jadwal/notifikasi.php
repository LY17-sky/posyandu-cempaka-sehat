...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama !== '' && $pesan !== '') {
        ...
        flash('message', 'Notifikasi WA telah dicatat.');
        ...
        <div class="rounded-3xl border border-emerald-100 bg-white/40 p-6 sm:p-8 backdrop-blur-md">
            <form method="post" class="grid gap-6 md:grid-cols-2">
                <label class="block md:col-span-2">
                    <span class="text-sm font-black text-emerald-900 ml-1 uppercase tracking-wider">Kepada</span>
                    <select name="nama" required class="mt-2 block w-full rounded-2xl border-emerald-100 bg-white px-5 py-4 text-emerald-950 focus:border-teal-400 focus:ring-teal-400/20 transition-all outline-none border-2">
                        <option value="">-- Pilih Penerima --</option>
                        <?php foreach ($balitas as $balita): ?>
                            <option value="<?php echo sanitize($balita['nama']); ?>"><?php echo sanitize($balita['nama']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </label>
